    ...
    <div class="container mx-auto px-4 py-8">
      ...
      <div v-else class="max-w-4xl mx-auto">
        ...
        <div class="card p-6 mb-6">
          ...
          <!-- Step 3: Delivery/Pickup Details -->
              ...
              <div>
                ...
                <div class="space-y-2">
                  <div v-for="item in cartStore.cartItems" :key="item.cartItemId" class="flex justify-between text-sm">
                    ...
                    <span class="font-semibold">{{ formatCurrency(item.basePrice * item.quantity) }}</span>
                  </div>
                </div>
              </div>
...
const paymentMethods = [
  { id: 'cash', name: 'Cash on Delivery', description: 'Pay with cash when your order arrives', icon: '💵' },
  { id: 'card', name: 'Credit/Debit Card', description: 'Secure payment with card', icon: '💳' },
  { id: 'wallet', name: 'Digital Wallet', description: 'PayPal, Apple Pay, Google Pay', icon: '📱' }
]
...
const validateStep = () => {
  ...
  if (currentStep.value === 0) {
    errors.value.name = validateField('Name', formData.value.name, { required: true, minLength: 2 })
    errors.value.phone = validateField('Phone', formData.value.phone, { required: true, phone: true })
    errors.value.email = validateField('Email', formData.value.email, { required: true, email: true })
  } else if (currentStep.value === 2 && formData.value.orderType === 'delivery') {
    errors.value.address = validateField('Address', formData.value.address, { required: true, minLength: 5 })
    errors.value.city = validateField('City', formData.value.city, { required: true })
    errors.value.zipCode = validateField('Zip Code', formData.value.zipCode, { required: true, zipCode: true })
  }
...
const submitOrder = async () => {
  ...
  try {
    const orderData = {
      ...
      deliveryAddress: formData.value.orderType === 'delivery' ? {
      ...
      pickupTime: formData.value.orderType === 'pickup'
